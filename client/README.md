...
**Rackphone Client** is the operator's end of a rack: it signs in to the gateway,
shows what arrived on each unit, sends a message back, moves a file, and puts a
unit's screen on yours. It is the only thing in this repository a person looks
...
stealing. It asks for the narrowest scope that does its job.
...
│ data      feed, files    │──files──►│ /sdcard/rackphone│
└──────────────────────────┘          └──────────────────┘
```
...
Nothing is configured at build time. The server address, the credentials and
every preference are entered in the app and stored on the device.

| Setting | Default | What it does |
| --- | --- | --- |
| `Server address` | — | The gateway's URL, `https://` in anything but a test |
| `Biometric unlock` | On | Asks for a fingerprint before the stored token is used |
| `Require biometrics for control` | On | Asks again before the screen and before sending |
| `Auto-lock` | 5 min | Idle time before the token is put away again |
| `Notify on SMS` / `calls` | On | Whether an arrival raises a local notification |
| `Notify on app notifications` | Off | Off because a rack unit produces hundreds a day |
...
lib/src/data/      what a unit has: the feed, its files
...
lib/src/ui/        the shell, the four destinations, the pages
android/.../       MediaCodec, the texture, and the document picker
```
