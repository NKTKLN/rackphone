...
def test_real_exposition_sample() -> None:
    ...
    out = add_unit_label(source, "lisa01")
...
class TestCollection:
    ...
    def test_reports_a_reachable_unit_as_up(
        ...
        collected = collect_metrics([unit])
        assert 'm{unit="lisa01"} 1' in collected
        assert 'rackphone_up{unit="lisa01"} 1' in collected

    @pytest.mark.usefixtures("repo")
    def test_unreachable_unit_is_exported_as_down(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        unit = units.create_unit("lisa01", "AAA")
        ...
        monkeypatch.setattr(exposition.adb, "resolve_serial", explode)
        collected = collect_metrics([unit])
        assert 'rackphone_up{unit="lisa01"} 0' in collected
        assert 'rackphone_collect_duration_seconds{unit="lisa01"}' in collected
